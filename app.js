...
function SeriesVideoDetail(item){const detail=item.detail||{};const custom=VideoNote(detail.note,detail.tools||[],detail.links||detail.link);const episodes=detail.episodes||[];return `<main class="video-detail series-detail"><div class="detail-toolbar"><a class="back" href="/video">← 返回视频作品</a></div><section class="detail-hero series-title"><h1>${esc(item.title)}</h1></section><section class="series-videos"><h2>系列视频</h2><div class="series-video-grid">${episodes.map((episode,index)=>`<figure><video controls playsinline preload="metadata"${episode.poster?` poster="${esc(episode.poster)}"`:''} aria-label="播放${esc(episode.title)}"><source src="${esc(episode.src)}#t=0.1" type="video/mp4">你的浏览器暂不支持视频播放。</video><figcaption><span>${String(index+1).padStart(2,'0')}</span>${esc(episode.title)}</figcaption></figure>`).join('')}</div></section>${custom}</main>`;}
