...
function VideoNote(note,tools=[],linkData){const links=Array.isArray(linkData)?linkData:linkData?[linkData]:[];if(!note&&!tools.length&&!links.length)return '';return `<section class="video-note">${note||links.length?`<div class="video-note-copy"><h2>制作说明</h2>${note?`<p>${esc(note)}</p>`:''}${links.length?`<div class="video-note-links">${links.map(link=>`<a class="video-note-link" href="${esc(link.url)}" target="_blank" rel="noopener noreferrer"${link.download?' download':''}>${esc(link.label||link.url)} <span aria-hidden="true">${link.download?'↓':'↗'}</span></a>`).join('')}</div>`:''}</div>`:''}${tools.length?`<div class="video-note-tools">${tools.map(tool=>`<div><span>${esc(tool.label)}</span><strong>${esc(tool.value)}</strong></div>`).join('')}</div>`:''}</section>`;}
...
function ProductionVideoGallery(items){if(!items?.length)return '';return `<section class="production-block production-demo-block"><p class="section-label">视频 DEMO</p><h2>技术演示与过程对比</h2><div class="production-demo-grid">${items.map(item=>`<figure><div class="production-demo-frame"><video controls playsinline preload="metadata" aria-label="播放${esc(item.title)}"><source src="${esc(item.src)}#t=0.1" type="video/mp4">你的浏览器暂不支持视频播放。</video></div><figcaption>${esc(item.title)}</figcaption></figure>`).join('')}</div></section>`;}
function SeriesVideoDetail(item){const detail=item.detail||{};const custom=VideoNote(detail.note,detail.tools||[],detail.links||detail.link);const metrics=DetailMetrics(detail.metrics||[]);const episodes=detail.episodes||[];return `<main class="video-detail series-detail"><div class="detail-toolbar"><a class="back" href="/video">← 返回视频作品</a></div><section class="detail-hero series-hero"><h1>${esc(item.title)}</h1><img class="series-cover" src="${esc(item.poster)}" alt="${esc(item.title)}封面"></section><section class="series-videos"><p class="section-label">系列视频</p><h2>抓马生活现场</h2><div class="series-video-grid">${episodes.map((episode,index)=>`<figure><video controls playsinline preload="metadata"${episode.poster?` poster="${esc(episode.poster)}"`:''} aria-label="播放${esc(episode.title)}"><source src="${esc(episode.src)}#t=0.1" type="video/mp4">你的浏览器暂不支持视频播放。</video><figcaption><span>${String(index+1).padStart(2,'0')}</span>${esc(episode.title)}</figcaption></figure>`).join('')}</div></section>${custom}${metrics}</main>`;}
function ProductionVideoDetail(item){
  ...
  const demoVideos=ProductionVideoGallery(production.demoVideos);
...
document.getElementById('app').innerHTML=Header(isHome)+(isHome?Home():videoItem?(videoItem.caseStudy==='executive-ip'?ExecutiveIpDetail(videoItem):videoItem.caseStudy==='production'?ProductionVideoDetail(videoItem):videoItem.caseStudy==='series'?SeriesVideoDetail(videoItem):VideoDetail(videoItem)):category?.id==='video'?VideoPage(category):category?.id==='writing'?WritingPage(category):category?.id==='projects'?ProjectsPage(category):category?.id==='visual'?VisualPage(category):category?Category(category):path==='/resume'?ResumePage():'<main class="category-page"><h1>页面未找到</h1><a class="button" href="/">返回首页</a></main>')+'<footer><span>© '+new Date().getFullYear()+' 林栖</span><span>内容 · 影像 · 创作</span></footer>';
...
if(profileCarousel){
  ...
  const preloadedPhotos=photos.map(src=>{const image=new Image();image.src=src;return image;});
  let photoIndex=0;
  const showProfilePhoto=index=>{
    photoIndex=(index+photos.length)%photos.length;
    photo.src=preloadedPhotos[photoIndex].src;
